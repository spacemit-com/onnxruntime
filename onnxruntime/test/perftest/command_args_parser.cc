...
ABSL_FLAG(std::string, e, "cpu", "Specifies the provider 'cpu','cuda','dnnl','tensorrt', 'nvtensorrtrtx', 'openvino', 'dml', 'acl', 'nnapi', 'coreml', 'qnn', 'snpe', 'rocm', 'migraphx', 'xnnpack', 'vitisai' , 'webgpu' or 'spacemit'.");
  ...
  // -e
  {
    ...
    if (!ep.empty()) {
      if (ep == "cpu") {
        ...
        test_config.machine_config.provider_type_name = onnxruntime::kSpaceMITExecutionProvider;
      } else if (ep == "cuda") {
