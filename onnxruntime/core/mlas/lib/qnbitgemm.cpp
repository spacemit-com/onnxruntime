...
)
{
    ...
    if (BlkBitWidth == 4 && Dispatch->SQ4BitGemmPackQuantBDataSizeWithScale != nullptr) {
        if (ScaleType == MLAS_QNBIT_GEMM_SCALE_TYPE::ScaleFp16) {
    ...
    } else if (BlkBitWidth == 4 && Dispatch->Q4BitGemmPackQuantBDataSize != nullptr) {
...
)
{
    ...
    if (BlkBitWidth == 4) {
        if (ComputeType == SQNBIT_CompInt8  && Dispatch->SQ4BitGemmPackQuantBDataAndBlkSumWithScale != nullptr) {
            const size_t BlockCountK = MlasDivRoundup(K, BlkLen);
            PackedQuantBDataStruct<float> packed_quant_b(PackedQuantBDataAndOrBlkSumWorkspace, N, BlockCountK, BlkLen);
            packed_quant_b.PackedQuantBData = static_cast<std::byte*>(PackedQuantBDataAndOrBlkSumWorkspace);
            Dispatch->SQ4BitGemmPackQuantBDataAndBlkSumWithScale(
                N,
                K,
                BlkLen,
                ComputeType,
        ...
        } else if (ComputeType == SQNBIT_CompInt8  && Dispatch->SQ4BitGemmPackQuantBDataAndBlkSum != nullptr &&
            ScaleType == MLAS_QNBIT_GEMM_SCALE_TYPE::ScaleFp32) {
            const size_t BlockCountK = MlasDivRoundup(K, BlkLen);
            PackedQuantBDataStruct<float> packed_quant_b(PackedQuantBDataAndOrBlkSumWorkspace, N, BlockCountK, BlkLen);
...
MlasQNBitGemmPackQuantBData(
    ...
    MlasQNBitGemmPackQuantBData(N, K, BlkBitWidth, BlkLen, ComputeType,
                                MLAS_QNBIT_GEMM_SCALE_TYPE::ScaleFp32, QuantBData,
                                PackedQuantBDataAndOrBlkSumWorkspace, QuantBScale,
                                has_zp_input, QuantBZeroPoint, ThreadPool);
}
...
AddBiasForGemm(const float* Bias, float* C, size_t CountM, size_t CountN, size_t ldc)
{
    ...
    for (size_t m = 0; m < CountM; m++) {
        ...
        float* sum = C;
        int64_t n = CountN;
        for (size_t vl; n > 0; n -= vl, sum += vl, bias += vl) {
            vl = __riscv_vsetvl_e32m4(n);
            vfloat32m4_t lhs = __riscv_vle32_v_f32m4(sum, vl);
            vfloat32m4_t rhs = __riscv_vle32_v_f32m4(bias, vl);
            vfloat32m4_t dst = __riscv_vfadd_vv_f32m4(lhs, rhs, vl);
            __riscv_vse32_v_f32m4(sum, dst, vl);
        }
        C += ldc;
    }
#else
...
)
{
    ...
    for (size_t n = 0; n < RangeCountN; n += CountN) {
        ...
        while (RowsRemaining > 0) {
            ...
            auto RowsHandled = GetMlasPlatform().QNBitGemmDispatch->SQ4BitGemmMNKernel_CompFp32(
...
{
    const size_t scale_stride = DataParams->ScaleType == MLAS_QNBIT_GEMM_SCALE_TYPE::ScaleFp16 ? sizeof(uint16_t) : sizeof(float);
...
#else
    const size_t ComputeBlockCountN = 128;
#endif
    for (size_t n = 0; n < RangeCountN; n += CountN) {
        ...
        if (GetMlasPlatform().QNBitGemmDispatch->SQ4BitGemmKernel_CompInt8WithScale != nullptr) {
            ...
            while (RowsRemaining > 0) {
                const auto RowsHandled = GetMlasPlatform().QNBitGemmDispatch->SQ4BitGemmKernel_CompInt8WithScale(
                    BlkLen,
                    a_row, b_col, b_col_scale, b_col_zp, c_blk, RowsRemaining, CountN, K, k_blks, ldc, bias, scale_stride
                );
